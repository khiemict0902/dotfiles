...
# Theme location
theme = "/home/khiempd/.config/rofi/launchers/type-6/style-7.rasi"
...
def cli_args():
    ...
    """
    args = sys.argv[1:]
    cmd = CONF.get('dmenu', 'dmenu_command', fallback="rofi -dmenu -i")
...
def dmenu_cmd(num_lines, prompt="  Networks", active_lines=None):
    ...
    commands = {"dmenu": ["-p", str(prompt)],
                "rofi": ["-dmenu", "-p", str(prompt), "-theme", theme],
                "bemenu": ["-p", str(prompt)],
                "wofi": ["-p", str(prompt)],
                "fuzzel": ["-p", str(prompt), "-l", str(num_lines), "--log-level", "none"]}
    command = shlex.split(CONF.get('dmenu', 'dmenu_command', fallback="rofi"))
